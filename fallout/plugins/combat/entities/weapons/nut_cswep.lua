...
function SWEP:DrawHUD()
	if CLIENT then
		...
		if AP and APMax then
			...
			surface.DrawOutlinedRect(posX, posY, boxX, textSizeY * 1.5, 1)
			posY = posY + textSizeY * 0.75
			nut.util.drawText(apString, textX, posY, Color(0,238,0,150), 1, 1, "nutCombatHUD")
		end
		...
		if IsValid(self.viewed) then
			...
			if (action) then
				...
				posY = posY + textSizeY
				--target string
				if !action.selfOnly then
					local targetString = ""

					local hp = self.viewed:getHP() and tonumber(self.viewed:getHP())
					local hpMax = self.viewed:getMaxHP() and tonumber(self.viewed:getMaxHP())
