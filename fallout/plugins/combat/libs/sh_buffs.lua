...
--goes through all buffs and returns the modifications to a specific thing, multiplicatively
function playerMeta:getBuffAttributeMult(buffAttribute)
	local modify = 1
	for k, buff in pairs(self:getBuffs()) do
		if(buff[buffAttribute]) then
			modify = modify * buff[buffAttribute]
...
hook.Add("nut_OnReceiveDamage", "nut_buffGetHit", function(client, dmg, dmgT, part)
	client:buffGetHit()
end)
